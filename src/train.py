...
from envs.rb_frozen_lake import RBFrozenLakeEnv
...
def make_env(_map=None, is_gui=False, truncate=False, random_reset=False):
    ...
    if _map is None:
        return RBFrozenLakeEnv(grid=None,
                               ...
                               reset_on_clear=_env_setting['reset_on_clear'],
                               )
    else:
        map_desc = np.asarray(_map, dtype='c')
        return RBFrozenLakeEnv(grid=map_desc,
                               render_mode=render_mode,
                               hole_penalty=_env_setting['hole_penalty'],
                               reset_map=random_reset,
                               frozen_prob=_env_setting['frozen_prob'],
