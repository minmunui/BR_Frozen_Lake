    ...
    * v0: Initial version release
    ...
    def generate_random_map(self):
        ...
        while not valid:
            ...
            start = (0, 0)
            goal = (self.n_row - 1, self.n_col - 1)
            _map[start] = b'S'
            _map[goal] = b'G'
            valid = is_valid(_map, start, goal)
    ...
    def reset(
        ...
        # resetonclear is used to reset the map when the agent reaches the goal
        if self.reset_on_clear:
            if self.cleared:
                self.set_grid(self.generate_random_map())
                print("Map Reset on Clear")
                print(self.desc)
                self.cleared = False
        elif self.reset_map:
            self.set_grid(self.generate_random_map())
